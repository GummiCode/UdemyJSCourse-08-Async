...
const getIDs = new Promise((resolve, reject) => {
  setTimeout(() => {
    ...
    outcome > 2 ? resolve([1, 2, 3, 4, "Benji"]) : reject(["Error"]);
  }, 1500);
});
...
// We then call the asynchronous function to invoke it.
// When we invoke the asynchronous function, we can access the final Resolve data from the final Promlise by putting a 'return' phrase at the end of the async function, the using the .then method to process it.

getRecipesAW().then(pub => console.log(`Remember: You can trust books ${pub}!`));
...
const gumballSelector = (gumballSelectorData) => {
  return new Promise((resolve, reject) => {
    setTimeout(
      (gumballSelectorData) => {
        const gumColor = gumballSelectorData.colors[(gumballSelectorData.selector-1)];
        console.log(`You got a ${gumColor} gumball!`);
        resolve(gumColor);
        reject("Error!")
      },
      1500,
...
gumballMachine().then(gumballColor => console.log(`${gumballColor} is my favourite color!`));
